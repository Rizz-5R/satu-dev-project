import { useState } from "react";
import { Hadispage } from "./pages/ulumulhadis/Hadispage";
import HomePage from "./pages/Home/HomePage";
import { BrowserRouter, Routes, Route } from "react-router-dom";
import { DetailMateriPage } from "./pages/ulumulhadis/detail/DetailMateriPage";
import { Materihadis } from "./pages/ulumulhadis/detail/Materihadis";
import QuizPage from "./pages/quiz/QuizPage";
import AboutPage from "./pages/about/AboutPage";
import Navbar from "./components/layout/Navbar";
import Footer from "./components/layout/Footer";
import QuranPage from "./pages/ulumulquran/QuranPage";
import ModuleQuranPage from "./pages/ulumulquran/ModuleQuranPage";
import LessonQuranPage from "./pages/ulumulquran/LessonQuranPage";

function App() {
  return (
    <BrowserRouter>
      <Navbar />
      <Routes>
        <Route path="/" element={<HomePage />} />
        <Route path="/hadispage" element={<Hadispage />} />
        <Route path="/materihadis" element={<Materihadis />} />
        <Route path="/materi/:slug" element={<DetailMateriPage />} />
        {/* halam quran */}
        <Route path="/quranpage" element={<QuranPage />} />
        <Route path="/modules/:moduleSlug" element={<ModuleQuranPage />} />
        <Route path="/modules/:moduleSlug/:lessonSlug" element={<LessonQuranPage />} />
        {/* Quiz */}
        <Route path="/quizpage" element={<QuizPage />} />
        <Route path="/aboutpage" element={<AboutPage />} />
      </Routes>
      <Footer />
    </BrowserRouter>
  );
}

export default App;
